...
impl BenchConfig {
    fn from_args() -> BenchResult<Self> {
        ...
        override_u64("LANE_BENCH_LARGE_MIB", &mut config.large_mib)?;
        override_u64("LANE_BENCH_WEAK_MIB", &mut config.weak_mib)?;
        override_f64("LANE_BENCH_WEAK_MBPS", &mut config.weak_mbps)?;
        override_u64("LANE_BENCH_WEAK_LATENCY_MS", &mut config.weak_latency_ms)?;
        override_u64("LANE_BENCH_WEAK_JITTER_MS", &mut config.weak_jitter_ms)?;
        override_usize("LANE_BENCH_CONCURRENT_FILES", &mut config.concurrent_files)?;
        override_u64("LANE_BENCH_CONCURRENT_MIB", &mut config.concurrent_mib)?;
        override_usize("LANE_BENCH_RANGE_WORKERS", &mut config.range_workers)?;
        override_u64("LANE_BENCH_INTERRUPTED_MIB", &mut config.interrupted_mib)?;
        config.validate()?;
        Ok(config)
    }
...
fn workspace_root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        ...
        .expect("lane-core must be inside the workspace crates directory")
        .to_path_buf()
}
...
impl BenchServer {
    async fn start(max_upload_bytes: u64) -> BenchResult<Self> {
        ...
        let server = Server::new(ServerConfig {
            device_name: "LANE benchmark".into(),
...
async fn run() -> BenchResult<()> {
    ...
    println!("LANE transfer benchmark ({})", config.profile);
    ...
    let report = BenchmarkReport {
        ...
        lane_version: env!("CARGO_PKG_VERSION").into(),
...
async fn send_upload(
    ...
    let boundary = "lane-benchmark-boundary";
    let prefix = format!(
...
fn print_help() {
    println!(
        "LANE transfer benchmark\n\n\
         Usage: cargo bench -p lane-core --bench transfer -- [options]\n\n\
         Options:\n  \
         ...
         Environment overrides:\n  \
           LANE_BENCH_LARGE_MIB\n  \
           LANE_BENCH_WEAK_MIB\n  \
           LANE_BENCH_WEAK_MBPS\n  \
           LANE_BENCH_WEAK_LATENCY_MS\n  \
           LANE_BENCH_WEAK_JITTER_MS\n  \
           LANE_BENCH_CONCURRENT_FILES\n  \
           LANE_BENCH_CONCURRENT_MIB\n  \
           LANE_BENCH_RANGE_WORKERS\n  \
           LANE_BENCH_INTERRUPTED_MIB"
    );
}
