...
mod tests {
    ...
    fn test_settings() -> Settings {
        Settings {
            ...
            host: "0.0.0.0".into(),
            port: 8080,
            receive_dir: r"C:\Temp\LANE".into(),
            max_upload_bytes: 10 * 1024 * 1024 * 1024,
        }
    }
    ...
    fn persists_and_reloads() {
        let dir = std::env::temp_dir().join(format!("lane-settings-{}", std::process::id()));
    ...
    fn malformed_port_values_do_not_block_startup() {
        ...
        for content in [
            ...
            let dir =
                std::env::temp_dir().join(format!("lane-settings-bad-{}", std::process::id()));
            let _ = std::fs::remove_dir_all(&dir);
            std::fs::create_dir_all(&dir).unwrap();
            let path = dir.join("settings.json");
