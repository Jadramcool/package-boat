//! 应用数据路径解析：`%LOCALAPPDATA%\LANE`（目录表、设置）与 `Downloads\LANE`（接收目录）。

use std::path::PathBuf;

/// Windows 上的应用数据目录名。
pub const DIRECTORY_NAME: &str = "LANE";
...
/// 解析默认路径。`data_dir` 位于 `%LOCALAPPDATA%\LANE`（无此变量时回退到系统配置目录），
/// `receive_dir` 位于 `{用户主目录}\Downloads\LANE`。
...
mod tests {
    ...
    fn default_paths_shape() {
        ...
        assert!(paths.receive_dir.ends_with("LANE"));
    }
}
