...
mod tests {
    ...
    async fn starts_and_stops_http_service() {
        ...
        let temp = std::env::temp_dir().join(format!("lane-host-{}", std::process::id()));
    ...
    async fn port_conflict_falls_back() {
        use crate::settings::Settings;
        use std::path::PathBuf;
        let temp = std::env::temp_dir().join(format!("lane-host-fb-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&temp);
        std::fs::create_dir_all(&temp).unwrap();
    ...
    async fn concurrent_start_only_starts_once() {
        use crate::settings::Settings;
        use std::path::PathBuf;
        let temp = std::env::temp_dir().join(format!("lane-host-cc-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&temp);
        std::fs::create_dir_all(&temp).unwrap();
