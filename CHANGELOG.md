...
- 桌面端「检查更新」无论成功、无新版本或失败，都会给出可见提示。
...
- 桌面命令条配对码旁支持手动刷新：热更新六位码、不重启服务；已配对设备会话保持有效，新设备需使用新码。

### Changed

- 桌面端与浏览器接收页的品牌字标由旧 “L” 占位替换为私域航迹图标。
- 桌面端内容区不再整页滚动：提示条常驻上方，左侧共享清单与右侧设置栏各自独立滚动。
- **安全边界**：移出共享清单 / 取消共享 / HTTP DELETE 一律不再删除磁盘文件。本地文件（inbox）与远程接收文件只从清单隐藏或移除记录，接收目录中的实体文件始终保留。
- 接收设置新增独立「重新扫描接收目录」按钮：把此前移出清单的本地文件加回来；与「共享目录内全部文件」开关键解耦，重启应用不会自动恢复。
