...
const totalSize = computed(() =>
  formatBytes(props.items.reduce((total, item) => total + item.size, 0)),
);
</script>
...
<template>
  <section
    ...
    :class="{ 'drag-active': props.dragActive }"
    aria-labelledby="desktop-list-title">
    ...
    <header class="list-header">
      <div class="list-heading">
        ...
        <p>
          {{ props.items.length }} 个文件 · {{ totalSize }} · 原位共享
          {{ props.linkedCount }} · 远程接收 {{ props.receivedCount
          }}<template v-if="props.inboxCount">
            · 本地文件 {{ props.inboxCount }}</template
          >
        </p>
      </div>
      <div class="list-tools">
        ...
        <!-- 文案单独成 span：匿名文本节点 + SVG 基线会让 CJK 在 flex 胶囊里偏心 -->
        <span v-if="props.items.length > 0" class="safety-note">
          <ShieldCheck :size="13" aria-hidden="true" />
          <span>清空只移除清单，不删除原文件</span>
        </span>
        <button
          class="clear-button"
          type="button"
...
.desktop-file-list.drag-active {
  border-color: var(--acid-deep);
  box-shadow: 0 12px 34px rgb(92 122 18 / 18%);
}
...
.desktop-file-list.drag-active .drop-zone {
  background: var(--acid-wash);
}
.desktop-file-list.drag-active .drop-zone::after {
  border-color: var(--acid-deep);
}
.drop-icon {
  position: relative;
...
.drop-zone button {
  ...
  color: var(--paper);
  cursor: pointer;
  font: 680 13.5px/1.2 var(--font-label);
  transition:
...
.drop-zone button:hover:not(:disabled) {
  background: #343c31;
  border-color: #343c31;
}
...
.list-heading h2 {
  margin: 0;
  font: 720 22px/1.1 var(--font-display);
...
/* 清空是低频破坏性操作，常态用中性描边与普通文字色——不能用危险色抢注意力，
   ...
   inline-flex 会覆盖基按钮的 grid/place-items，必须补 align/justify。 */
.list-tools .clear-button {
  width: auto;
  display: inline-flex;
  align-items: center;
  justify-content: center;
  ...
  padding: 0 11px;
  color: var(--muted);
  font: 650 12.5px var(--font-label);
}
.list-tools .clear-button svg {
  flex: none;
  display: block;
}
.list-tools .clear-button:hover:not(:disabled) {
  border-color: var(--signal);
  background: var(--signal);
  color: #fff;
}
.list-tools .clear-button:disabled {
  cursor: not-allowed;
  opacity: 0.45;
}
/* 安全承诺：acid-wash 洗色底 + acid-deep 文字（已加深到 5.84:1，过 AA）
   文案用独立 flex 子项 + svg display:block，避免 CJK 匿名文本/基线造成的垂直偏心 */
.safety-note {
  display: inline-flex;
  align-items: center;
  justify-content: center;
  ...
  background: var(--acid-wash);
  color: var(--acid-deep);
  font: 650 11.5px var(--font-label);
  white-space: nowrap;
}
.safety-note svg {
  flex: none;
...
.table-header {
  ...
  grid-template-columns:
    minmax(260px, 1.5fr) minmax(82px, 0.42fr) 152px minmax(100px, 0.48fr)
    68px;
  align-items: center;
  gap: 16px;
  padding: 11px 16px;
...
@media (max-width: 1200px) {
  .table-header {
    grid-template-columns: minmax(250px, 1fr) 90px 100px 68px;
    gap: 16px;
  }
  .date-column {
    display: none;
...
@media (max-width: 900px) {
  ...
  .table-header {
    grid-template-columns: minmax(250px, 1fr) 90px 68px;
    gap: 12px;
    padding-right: 14px;
    padding-left: 14px;
  }
